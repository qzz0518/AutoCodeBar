...
- **Guided Accessibility access**: the same drag-in guide card as Full Disk Access — System Settings opens, the card flies alongside the window, you drag AutoCodeBar from the card into the list, and the card dismisses itself once the grant takes effect.
- **Accessibility row on the Permissions page**: listed next to Full Disk Access and Notifications, so the current state is visible at a glance.
